...
fn upstream_panics_return_error() {
    let cases: &[(&str, office2pdf::config::Format)] = &[
        ...
        ("libreoffice/chart_hyperlink.xlsx", office2pdf::config::Format::Xlsx),
        // umya-spreadsheet: ParseFloatError on boolean cell
        ("libreoffice/check-boolean.xlsx", office2pdf::config::Format::Xlsx),
    ];
    for (name, format) in cases {
        ...
        assert!(
            result.is_err(),
            "{name} should return Err (not panic)"
        );
    }
}
